...
class AutoCut:
    def __init__(self, input_file, output_base_name=None, config=None):
        ...
        """
        try:
            sub.check_call(['ffmpeg', '-h'], stdout=sub.DEVNULL, stderr=sub.DEVNULL)
            sub.check_call(['ffprobe', '-h'], stdout=sub.DEVNULL, stderr=sub.DEVNULL)
        except FileNotFoundError:
            print('error: could not find ffmpeg/ffprobe', file=sys.stderr)
            sys.exit(1)
    ...
    def run_montage(self, rms_threshold):
        ...
        """
        segments = self.audio_level_segmentation(self.input_file, rms_threshold)
        if not segments:
    ...
    def audio_level_segmentation(self, filepath, threshold):
...
if __name__ == '__main__':
    try:
        ...
        print('error: caught unexpected exception')
        print(e)
        raise e